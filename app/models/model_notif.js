module.exports = {
	get_all_unread_admin : function (con, cb) {
		con.query(
            {
                sql : `
                	SELECT * FROM notif 
	...
	get_all_unread_by_user : function (con, cred , cb) {
		con.query(
            {
                sql : `
                	SELECT * FROM notif 
	...
	get_all_read_by_user : function (con, cred, cb) {
		con.query(
            {
                sql : `
                	SELECT * FROM notif 
	...
	update_notif_to_read_state : function (con, cred, cb) {
		con.query(
            {
                sql : `
                	UPDATE notif SET ?
	...
	save : function (con,cred, cb) {
		con.query({
            sql : `INSERT INTO notif SET ?`,
            values : [{
                'notif_user_id' : cred.notif_user_id,
                'notif_user_role' : cred.notif_user_role,
                'notif_msg' : cred.notif_msg,
                'notif_user_destiny' : cred.notif_user_destiny,
            },function (err, res, fields) {
                var r= resData("simpan notif ", err, res, fields);
                cb(r);
            }]
        })
	}
